...
case object Include extends ((LispList, Environment) => LispValue) {
  /** We need to include the input source and the filepath of every expressions.
    * 
    * The source contains the line and column number, and the string representing the expression itself. 
    * We need them for the construction of the error messges later on. The first string represents the 
    * parser input, and the second one would represent the filepath.
    */
  type LispListWithSources = List[(LispValue, String, String)]
  type LoadResult = (LispListWithSources, Option[LispError])
  
  def apply(args: LispList, environment: Environment) = withStringArg(args, environment) { filepath =>
    /** This function gets called before the evaluation of every expressions in the source file.
      * 
      * It has three main goals:
      * 1. Expand all the "include" expressions. That is, if an "include" is found, then apply preInclude to
      *   the file specified in the include expression. This is a recursive process. Duplicates of the "include"
      *   found in the other source files are ignored.
      * 2. Parse all the expressions without evaluating them. The evaluation should happen later because we
      *   need to register all the function and macro declarations to the environment first.
      * 3. Remove the other main functions that are not defined inside the main source file (the path you
      *   first loaded).
      */
    def preInclude(path: String, pathsIncluded: List[String]): LoadResult = {
      if (Files.exists(Paths.get(path))) {
        ...
        def recurseRead(input: LispParser.Input, acc: LispListWithSources): LoadResult =
          if (input.atEnd) (acc, None)
            ...
            case success @ Success(expr, next) => expr match {
              ...
              case ConsLispList(LispSymbol(Keywords.Import), ConsLispList(LispString(path1), NilLispList)) =>
              ...
              // Skip the main functions that are not in the main file.  
              case ConsLispList(LispSymbol(Keywords.Defun), ConsLispList(LispSymbol("main"), _)) 
                    if path != filepath =>
                recurseRead(next, acc)
        ...
        recurseRead(new CharSequenceReader(Source.fromFile(path).mkString.trim), Nil)
      } else (Nil, Some(Errors.fileNotFound(path)))
    }
    
    preInclude(filepath, Nil) match {
              ...
              def recurse(funcsWithSources: LispListWithSources,
                ...
                funcsWithSources match {
                  ...
                  case (ConsLispList(LispSymbol(op),
                    ...
                    lambda(params !: body !: NilLispList, environment) match {
